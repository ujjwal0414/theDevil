...
const NavigationPanel=({show,setPanelShow,usrExs,setExs})=>{
    let navigate=useNavigate()
    let location=useLocation();
    return(
        <>{
          <div className={`absolute transition-all z-40 px-4 pt-6 duration-100 border-2 shadow-slate-900   shadow-md ${show?"block bottom-0":"hidden"} md:hidden w-[100%] rounded-t-3xl min-h-[20vh] pb-3 bg-white`}>
